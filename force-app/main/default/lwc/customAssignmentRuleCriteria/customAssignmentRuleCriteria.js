...
import createAssignmentCritera from '@salesforce/apex/CustomAssignmentRuleCntrl.createAssignmentCriterias';
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import { getPicklistValues } from 'lightning/uiObjectInfoApi';
import OPERATOR_FIELD from '@salesforce/schema/Assignment_Rule_Criteria__c.Operator__c';
import { NavigationMixin } from 'lightning/navigation';

export default class CustomAssignmentRuleCriteria extends NavigationMixin(LightningElement) {
    ...
    allPicklistValues({ error, data }) {
        if(data){
            console.log('JSON pick'+JSON.stringify(data));
    ...
    allusers({error,data})
    {
        if(data){
            ...
            console.log('JSON user data error'+JSON.stringify(error));
        }
    }
    ...
    /*@wire(getPicklistValues, { recordTypeId: '012000000000000AAA', fieldApiName: OPERATOR_FIELD })
    allPicklistValues({ error, data }) {
        if(data){
            console.log('JSON pick'+JSON.stringify(data));
            this.operatorvalues = data.map(opvalue => {
    ...
    allFields({ error, data }) {
        if(data){
            ...
            console.log('JSON data field1'+JSON.stringify(this.allSobjectList));
        }
        else{
